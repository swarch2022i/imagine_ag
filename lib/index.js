...
const resolvers$1 = {
  Query: {
    allImageStorage: (_) => getRequest(URL$1, ''),
    imageStorageById: async(_, { imageStorageId }) => {
      console.log(`${URL$1}/${imageStorageId}`);
      await generalRequest(`${URL$1}/${imageStorageId}`, 'GET');
    },
  },
  Mutation: {
    deleteImageStorage: (_, { imageStorageId }) => generalRequest(`${URL$1}/${imageStorageId}`, 'DELETE'),
  },
};
...
var routerStorage$1 = () => {
  ...
  routerStorage.post('/', upload.array('images'), async(ctx) => {
    if (!ctx.files) {
      ...
      let response = await axios.post(`${URL$2}/upload`, formData, {
        headers: {
