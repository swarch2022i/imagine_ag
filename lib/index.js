...
const resolvers$1 = {
  Query: {
    collection: async(_, { uid }) => {
      console.log(URL$1);
      return await getRequest(`${URL$1}`, 'collection', { uid })},
    userCollections: (_, { uid }) => getRequest(`${URL$1}`, 'user', { uid }),
  },
  Mutation: {
    createCollection: (_, { collection }) => generalRequest(`${URL$1}/collection`, `POST`, collection),
    updateCollection: (_, { uid_owner, uid_collection, description, images_ids }) => generalRequest(`${URL$1}/collection`, `PATCH`, { uid_owner, uid_collection, description, images_ids }),
    deleteCollection: (_, { uid_owner, uid_collection }) => generalRequest(`${URL$1}/collection`, `DELETE`, { uid_owner, uid_collection }),
    createUser: (_, { user }) => generalRequest(`${URL$1}/user`, `POST`, user),
  },
};
